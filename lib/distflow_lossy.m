...
%%%                     result.branch(:, QF) = Qf
%%% 
%%%  REFERENCES: Schweitzer, Eran, Shammya Shananda Saha, Anna Scaglione, Nathan G Johnson, and Daniel Arnold. 2019.
%%%     “Lossy DistFlow Formulation for Single and Multiphase Radial Feeders.” IEEE Transactions on Power Systems.
%%%
%%% Notes:
%%%     alpha = 0.5: lossless case
%%%     case must be radial
%%%     bus number mush be consequtively numbered
%%%     the largest numberred bus is the substation 
...
% end
substation = max(mpc.bus(:,BUS_I));
if length(substation) ~= 1
    error('currently only supports distribution grid connecting with one substation.');
end

not_substation = setdiff(1:nb, substation);
...
M0 = F-T;
% M = M0(:,2:end); % this step selects all non-substation nodes
M = M0(:,not_substation);
...
slack_voltage = mpc.bus(substation, VM);
...
% construct result struct output
result = mpc;
% result.bus = [result.bus, zeros( size(mpc.bus,1), 4)]; % [LAM_P, LAM_Q, MU_VAX, MU_VIN]
% result.branch = [result.bus, zeros( size(mpc.bus,1), 4)]; % [LAM_P, LAM_Q, MU_VAX, MU_VIN]
result.bus(:,VM) = vm;
result.branch(:,PF) = Pf;
result.branch(:,QF) = Qf;
result.branch(:,PT) = -Pf;
